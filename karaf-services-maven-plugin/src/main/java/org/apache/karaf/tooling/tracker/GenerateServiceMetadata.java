...
public class GenerateServiceMetadata extends AbstractMojo {
	...
	/**
	 * The classloader to use for loading the commands. Can be "project" or
	 * "plugin"
	 */
	@Parameter(defaultValue = "project")
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		getLog().info("START karaf-services-maven-plugin artifactInclude: " + artifactInclude);
		...
		getLog().info("STOP karaf-services-maven-plugin");
	}
			...
			for (Artifact artifact : project.getArtifacts()) {
				if (artifactInclude != null && artifactInclude.length() > 0
					...
					if (file != null) {
						...
						getLog().debug("   classpath: " + file);
					}
				} else {
					getLog().info(" Ignore Artifact " + artifact.getGroupId() + "/" + artifact.getArtifactId());
				}
			}

			ClassLoader loader = new URLClassLoader(urls.toArray(new URL[urls.size()]), getClass().getClassLoader());
			getLog().info("Load Classes; " + urls.size() + " jars");
