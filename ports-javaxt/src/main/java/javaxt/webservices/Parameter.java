...
public class Parameter {
	 ...
	 * 
	 * @param val
	 *            Accepts either an array or a single instance of byte, string,
	 *            boolean, double, float, int, short, long, decimal,
	 *            java.util.Date, javaxt.utils.Date, java.io.File, and
	 *            javaxt.io.File.
	 */
		...
		if (isComplex()) {
			...
			if (val.getClass().isArray()) {
				...
				if (getMaxOccurs() > 1) {
					...
					// Copy all the objects in the array. Covert types as
					// needed.
					Object[] a = (Object[]) val;
					...
					if (val instanceof byte[]) {
						...
						// throw new Exception("This parameter does not support
						// arrays.");
					}
	...
	private Object getValue(Object val) {
		if (type.equalsIgnoreCase("base64Binary")) {
			...
			if (val instanceof java.util.Date) {
				...
				try {
					return new javaxt.utils.Date((String) val).getDate();
				} catch (Exception e) {
				}
			}
		...
		if (value != null && value.getClass().isArray()) {
			...
			if (value instanceof byte[]) {
				...
				for (Object obj : (Object[]) value) {
					String val = obj + "";
					if (escapeXML)
						val = javaxt.xml.DOM.escapeXml(val);
				 ...
				 * "<item xsi:type=\"xsd:string\">SUN</item> " + "</name>";
				 */
