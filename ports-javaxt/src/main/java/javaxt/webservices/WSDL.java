...
public class WSDL {
	...
	private class Element {
		...
		public String toString() {
			...
			xml.append("   <parameter " + "name=\"" + Name + "\" " + "type=\"" + Type + "\" " + "minOccurs=\""
			        + minOccurs + "\" " + "maxOccurs=\"" + maxOccurs + "\" " + "isattribute=\"" + IsAttribute + "\" "
			        + "iscomplex=\"" + IsComplex + "\" " + "isnillable=\"" + IsNillable + "\">" + vbCrLf);
	...
	// <editor-fold defaultstate="collapsed" desc="Core WSDL Parser. Click on
	...
	private void parseWSDL() {
		...
		String SSD = "<?xml version=\"1.0\" encoding=\"utf-8\"?>" + vbCrLf + "<ssd>" + vbCrLf;
		...
		for (int i = 0; i < Definitions.getLength(); i++) {
			if (contains(Definitions.item(i).getNodeName(), "service")) {
				...
				if (Port != null) {
					...
					SSD += " <service name=\"" + ServiceName + "\" url=\"" + Port.Address + "\" namespace=\""
					        + getTargetNameSpace() + "\">" + vbCrLf;
					...
					for (int j = 0; j < arrBindings.size(); j++) {
						...
						SSD += "  <method name=\"" + Binding.Operation + "\"" + SoapAction + " resultsNode=\""
						        + ResultsNode + "\">" + vbCrLf;
	...
	private void parseComplexNode(Node node, String ElementName, Element parentElement) {
		...
		if (nodeName.equals("element")) {
			 ...
			 * Choices choices = new Choices(); java.util.Iterator<Element> it =
	...
	// <editor-fold defaultstate="collapsed" desc="Public Members. Click on the
