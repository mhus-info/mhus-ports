...
public class Database implements Cloneable {
	 ...
	 * 
	 * @param host
	 *            Server name/port (e.g. localhost:9080) or a path to a file
	...
	protected String getURL() {
		...
		if (vendor.equals("Sybase")) {
			if (path.toLowerCase().contains("tds:") == false) {
	...
	// **************************************************************************
	/**
	 * Used to set the ConnectionPoolDataSource for the database. Typically, the
	 * getConnectionPoolDataSource() method is used to create a
	...
	public ConnectionPoolDataSource getConnectionPoolDataSource() throws SQLException {
		...
		if (driver.equals("sqlite")) {
			...
			/*
			 * javax.sql.DataSource sqliteDS = new DataSource(); sqliteDS.setURL
			 * ("jdbc:sqlite://" + name); dataSource = sqliteDS;
			 */
	...
	public static String[] getReservedKeywords(Connection conn) {
		...
		if (driver.equals("Firebird")) {
			...
			// Try to get reserved keywords from the database. Note that in
			// PostgreSQL
			// "non-reserved" keywords are key words that are explicitly known
			// to
			// the parser but are allowed as column or table names. Therefore,
			// we
			...
			if (pgKeywords == null) {
				...
				try {
					stmt = conn.getConnection().createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY,
	...
	private static final String[] fbKeywords = new String[] { "ADD", "ADMIN", "ALL", "ALTER", "AND", "ANY", "AS", "AT",
	        "AVG", "BEGIN", "BETWEEN", "BIGINT", "BIT_LENGTH", "BLOB", "BOTH", "BY", "CASE", "CAST", "CHAR",
	        "CHAR_LENGTH", "CHARACTER", "CHARACTER_LENGTH", "CHECK", "CLOSE", "COLLATE", "COLUMN", "COMMIT", "CONNECT",
	        "CONSTRAINT", "COUNT", "CREATE", "CROSS", "CURRENT", "CURRENT_CONNECTION", "CURRENT_DATE", "CURRENT_ROLE",
	        "CURRENT_TIME", "CURRENT_TIMESTAMP", "CURRENT_TRANSACTION", "CURRENT_USER", "CURSOR", "DATE", "DAY", "DEC",
	        "DECIMAL", "DECLARE", "DEFAULT", "DELETE", "DISCONNECT", "DISTINCT", "DOUBLE", "DROP", "ELSE", "END",
	        "ESCAPE", "EXECUTE", "EXISTS", "EXTERNAL", "EXTRACT", "FETCH", "FILTER", "FLOAT", "FOR", "FOREIGN", "FROM",
	        "FULL", "FUNCTION", "GDSCODE", "GLOBAL", "GRANT", "GROUP", "HAVING", "HOUR", "IN", "INDEX", "INNER",
	        "INSENSITIVE", "INSERT", "INT", "INTEGER", "INTO", "IS", "JOIN", "LEADING", "LEFT", "LIKE", "LONG", "LOWER",
	        ...
	        "ROLLBACK", "ROW_COUNT", "ROWS", "SAVEPOINT", "SECOND", "SELECT", "SENSITIVE", "SET", "SIMILAR", "SMALLINT",
	 ...
	 */
	private static final String[] msKeywords = new String[] { "ADD", "ALL", "ALTER", "AND", "ANY", "AS", "ASC",
	        "AUTHORIZATION", "BACKUP", "BEGIN", "BETWEEN", "BREAK", "BROWSE", "BULK", "BY", "CASCADE", "CASE", "CHECK",
	        ...
	        "DEFAULT", "DELETE", "DENY", "DESC", "DISK", "DISTINCT", "DISTRIBUTED", "DOUBLE", "DROP", "DUMP", "ELSE",
	        "END", "ERRLVL", "ESCAPE", "EXCEPT", "EXEC", "EXECUTE", "EXISTS", "EXIT", "EXTERNAL", "FETCH", "FILE",
	...
	private static String[] ansiKeywords = new String[] { "ABSOLUTE", "ACTION", "ADD", "AFTER", "ALL", "ALLOCATE",
