...
public class Recordset {
	...
	public java.sql.ResultSet open(String sqlString, Connection Connection, boolean ReadOnly) throws SQLException {
		 ...
		 * i<exposedElements.length; i++){ String element = exposedElements[i];
		 * if (javaxt.utils.string.hasUpperCase(element)){ wrapElements = true;
		 * break; } } wrapElements = false; if (wrapElements){ sqlString =
		 * sqlParser.addQuotes(); //System.out.println(sqlString); } }
		 * catch(Exception e){ System.out.println("WARNING: Failed to parse SQL"
		 * ); e.printStackTrace(); } }
		 */
		...
		// Read-Write Connection
			...
			try {
				...
				// SQLite Connection
					...
					if (fetchSize != null)
						Conn.setAutoCommit(false);
					stmt = Conn.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY); // xerial
					                                                                                      // only
					                                                                                      // seems
					                                                                                      // to
					                                                                                      // support
				...
				// DB2 Connection
					...
					try {
						...
						// System.out.println("createStatement(rs.TYPE_SCROLL_SENSITIVE,rs.CONCUR_UPDATABLE)
						// Error:");
					...
					if (rs == null) {
						try {
							...
							// System.out.println("createStatement(rs.TYPE_FORWARD_ONLY,rs.CONCUR_UPDATABLE)
							// Error:");
							// System.out.println(e.toString());
						}
					}
				...
				// Default Connection
					 ...
					 * java.sql.Statement.RETURN_GENERATED_KEYS); rs =
					 * stmt.getResultSet();
					 */
					State = 1;
				}
				...
				// System.out.println("ERROR Open RecordSet (RW): " +
				// e.toString());
				throw e;
			}
	...
	public void update() throws java.sql.SQLException {
		...
		if (batchSize > 1) {
			if (batchedStatements == null)
				batchedStatements = new java.util.HashMap<String, java.sql.PreparedStatement>();
