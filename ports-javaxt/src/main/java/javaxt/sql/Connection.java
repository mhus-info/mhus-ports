...
public class Connection {
	...
	// **************************************************************************
	/**
	 * Used to retrieve the time it took to open the database connection (in
	 * milliseconds)
	 */
	...
	public boolean open(Database database) throws java.sql.SQLException {
		...
		// Load JDBC Driver
		java.sql.Driver Driver = database.getDriver().load();
	...
	public boolean open(java.sql.Connection conn) {
		...
		try {
			...
			// System.out.println(database.getDriver().getVendor() + " ERROR: "
			// + e.toString());
			isClosed = true;
		}
