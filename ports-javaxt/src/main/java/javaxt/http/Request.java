...
public class Request extends MLog {
	...
	private void initHeaders() {
		...
		// this.setHeader("User-Agent", "Mozilla/5.0 (Windows; U; Windows NT
		// 5.1; en-US; rv:1.9.2.10)"); //windows xp
		this.setHeader("User-Agent", "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:57.0) Gecko/20100101 Firefox/57.0"); // windows
		                                                                                                               // 7
		// this.setHeader("User-Agent", "Mozilla/5.0 (Windows NT 6.3; WOW64;
		// rv:34.0) Gecko/20100101 Firefox/34.0"); //windows 8
	}
	 ...
	 * encounters a redirect. See setNumRedirects() for more information. Use
	 * the getInitialURL() to get the original URL used to instantiate this
	 * class.
	 */
	...
	// **************************************************************************
	/**
	 * Returns the URL used to instantiate this class. This URL may be different
	 * from the URL used to execute the request. See getURL() for more
	 * information.
	 */
	...
	// **************************************************************************
	/**
	 * Used to enable/disable certificate validation for HTTPS Connections. By
	 * default, certificates are validated via a Certificate Authority (CA).
	...
	private String getCredentials() throws Exception {
		if (username == null || password == null)
			...
			return javaxt.utils.Base64.encodeBytes((username + ":" + password).getBytes("UTF-8"));
	}
	...
	private class ConnectTimeout implements Runnable {
		...
		public void run() {
			...
			if (responseCode == -1) {
				con.disconnect();
				// System.out.println("** Timer thread forcing to quit
				// connection");
			}
		}
	}
	 ...
	 * 
	 * @param payload
	 *            InputStream containing the body of the HTTP request.
	 */
	 ...
	 * 
	 * @param payload
	 *            Byte array containing the body of the HTTP request.
	 */
	...
	public void write(javaxt.html.Input[] inputs) {
		...
		try {
			...
			for (int i = 0; i < inputs.length; i++) {
				...
				String contentDisposition = "Content-Disposition: form-data; name=\"" + input.getName() + "\""
				        + fileName;
				byte[] md = (contentDisposition + "\r\n" + contentType + "\r\n").getBytes("UTF-8");
				metadata.add(md);
				size += ((i > 0 ? 2 : 0) + 2 + boundarySize + 2); // CRLF + 2
				                                                  // hyphens +
				                                                  // boundary +
				                                                  // CRLF
				size += md.length + inputLength;
			}
			size += (boundarySize + 8); // CRLF + 2 hyphens + boundary + 2
			                            // hyphens + CRLF
	...
	private URLConnection connect(boolean doOutput) {
		try {
			...
			// By default, Java 1.6 and earlier use the SSLv2 protocol to
			// initiate
			// an SSL handshake. For security reasons, most modern webservers
			// will
			...
			// Encode whitespaces and other illegal chars using the javaxt URL
			// class
	...
	private URLConnection getConnection(boolean doOutput) {
		...
		// simply open a connection and let the caller figure out what to do
		// with
		// the response.
		...
		// socket. Therefore, for "POST" requests, we must open a writable
		// socket
	...
	private void parseResponse(URLConnection conn) {
		...
		headers = conn.getHeaderFields(); // <-- Once this is called, clients
		                                  // can no longer write to the socket!
		if (!headers.isEmpty()) {

			List status = headers.get(null);
			if (status != null) {
				...
				if (st.hasMoreTokens())
					responseCode = javaxt.utils.string.toInt(st.nextToken().trim());
				if (st.hasMoreTokens()) {
					message = "";
	...
	private Long getExpiration(URLConnection connection, long baseTime) {
		...
		DateFormat PATTERN_RFC1123 = new SimpleDateFormat("EEE, dd MMM yyyy HH:mm:ss zzz"); // ,
		                                                                                    // Locale.US
	...
	public Proxy setProxy(String proxyHost, int proxyPort) {
		SocketAddress proxyAddr = InetSocketAddress.createUnresolved(proxyHost, proxyPort); // new
	...
	public boolean isProxyAvailable(String proxyHost, int proxyPort) {
		...
		try {
			...
			log().d("Name: ", address.getHostName(), "Addr: ", address.getHostAddress(), "Reach: ",
	...
	public String toString() {
		...
		if (requestHeaders != null) {
			...
			while (it.hasNext()) {
				...
				if (key != null) {
					java.util.List list = requestHeaders.get(key);
