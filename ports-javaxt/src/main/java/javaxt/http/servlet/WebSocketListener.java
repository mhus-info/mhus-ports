...
public class WebSocketListener  {
  ...
  //**************************************************************************
    ...
    throws ServletException, IOException {
      ...
      //Validate request
        if (!request.isWebSocket()) throw new ServletException("Invalid WebSocket request");
      ...
      //Upgrade the request
        try{
            ...
            if (seckey == null) throw new Exception("Missing Sec-WebSocket-Key");
      ...
      //Destroy the response object. We'll be handling the response from here on out...
        response.reset();
   ...
   * 
   *  @param remote Used to indicate who is initiating the close event. If 
   *  true then the client initiated the close event. Otherwise, it is the 
   *  server.
   */
        ...
        if (curop == Frame.Opcode.CLOSING) {
            ...
            if (curop != Frame.Opcode.CONTINUOUS ) {
                if( current_continuous_frame != null )
                        throw new FrameException( CloseFrame.PROTOCOL_ERROR, "Previous continuous frame sequence not completed." );
                ...
                if( current_continuous_frame.getOpcode() == Frame.Opcode.TEXT ) {
                    ((Frame) current_continuous_frame).setPayload( getPayloadFromByteBufferList() );
                    ((Frame) current_continuous_frame ).isValid();
                    try {
