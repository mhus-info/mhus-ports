...
//******************************************************************************
/**
 * Used to read raw bytes sent from the client to the server. Assumes the data
 * is a valid HTTP/1.1 request. Supports both http and https (ssl/tls). This
 * class implements the javax.servlet.http.HttpServletRequest interface defined
 * in Version 2.5 of the Java Servlet API.
 *
 ******************************************************************************/
...
public class HttpServletRequest extends MLog implements javax.servlet.http.HttpServletRequest {
	...
	private static final String[] methods = new String[] { "GET", "POST", "HEAD", "PUT", "OPTIONS", "TRACE", "DELETE" };
	 ...
	 * empty line.
	 */
	public HttpServletRequest(SocketConnection connection, HttpServlet servlet) throws ServletException, IOException {
		...
		String[] header = new String[30]; // <--max header length is set to 30
		                                  // lines
		char[] row = new char[2048]; // <--max row length is set to 2048
		                             // characters
		int i = 0;
		int j = 0;
		...
		while (true) {
			...
			if (totalBytesRead == 1 && ((a > 19 && a < 25) || a == -128)) {
				...
				int tlsVersion = recordHeader.get(1);
				if (a == -128)
					tlsVersion = recordHeader.get(3);
				if (tlsVersion != 3)
	...
	private void parseHeader() throws IOException {
		...
		// Parse the first line of the header to get the method, path, and
		// version
	 ...
	 *
	 * @param name
	 *            A String specifying the header name (e.g. "Content-Encoding")
	 ...
	 *
	 * @param name
	 *            A String specifying the header name (e.g. "Accept-Language").
	 *            The header name is case insensitive.
	 */
	@Override
	 ...
	 *
	 * @param name
	 *            A String specifying the header name (e.g. "Content-Length").
	 *            The header name is case insensitive.
	 */
	@Override
	 ...
	 *
	 * @param name
	 *            A String specifying the header name (e.g.
	 *            "If-Modified-Since"). The header name is case insensitive.
	 */
	@Override
	public long getDateHeader(String name) throws IllegalArgumentException {
		...
		java.text.SimpleDateFormat formatter = new java.text.SimpleDateFormat(format, Locale.US); // locale?
	...
	// **************************************************************************
	/**
	 * Returns the "Content-Type" defined in the request header. Returns null if
	 * the "Content-Type" is not defined.
	 */
	@Override
	 ...
	 * provide an Accept-Language header, this method returns the default locale
	 * for the server.
	 */
	@Override
	 ...
	 * acceptable to the client based on the Accept-Language header. The list of
	 * Locales is ordered, starting with the preferred locale. If the client
	...
	public java.util.Enumeration<Locale> getLocales() {
		...
		if (locales.isEmpty())
			locales.put(Locale.getDefault(), 0D);
	 ...
	 * 
	 * <pre>
	 * GET /index.html?abc=123 HTTP/1.1
	 * </pre>
	 * 
	 ...
	 * the value of the part before ":" in the "Host" header, header value, if
	...
	private boolean isUpgradeRequest() {
		...
		if (!getProtocol().equals("HTTP/1.1"))
			return false;
	 ...
	 * <i>protocol/majorVersion.minorVersion</i> (e.g. "HTTP/1.1").
	 */
	@Override
	...
	public String getScheme() {
		...
		if (isWebSocket())
			protocol = protocol.equals("https") ? "wss" : "ws";
		return protocol;
	}
	 ...
	 *
	 * @param key
	 *            Parameter name. Performs a case insensitive search for the
	 *            keyword.
	 *
	 * @return Returns a comma delimited list of values associated with the
	 *         given key or a null value if the key is not found or if the value
	 *         is an empty string.
	 */
	@Override
	 ...
	 * parameters.
	 *
	 * public StringBuffer getRequestURL(){ String url = this.url.toString(); if
	 * (url.contains("?")) url = url.substring(0, url.indexOf("?")); return new
	...
	public byte[] getBody() throws IOException {
		...
		while (true) {
			...
			// Update the buffer size as needed. This will help avoid a 15
			// second
			// wait period when reading bytes from the socket.
			if (contentLength > 0) {
				int remaining = contentLength - bas.size();
				if (remaining < bufferSize)
					buf = ByteBuffer.allocateDirect(remaining);
			}
			...
			if (numBytesRead > 0) {
				...
				bas.write(b);
				if (contentLength < 0 && numBytesRead != bufferSize)
					break; // <-- Is this a valid use case? Can Content-Length
					       // be undefined?
				if (bas.size() == contentLength)
					break;
			} else {
				break;
			}
	...
	public ServletInputStream getInputStream() {
		if (inputStream == null)
			inputStream = new ServletInputStream(this);
		return inputStream;
	}
	 ...
	 * Automatically decrypts the body if the data is SSL/TLS encrypted. Either
	 * this method or getInputStream() may be called to read the body, but not
	 * both.
	 */
	@Override
	 ...
	 * potentially more memory efficient than calling getBody() and parsing the
	 * entire byte array. This is especially true when processing
	 ...
	 * getFormInputs() method. Note how easy it is to identify an uploaded file
	 * and save it to disk.
	 * 
	 * <pre>
	...
	public HttpSession getSession(boolean create) {
		...
		if (sessionID != null)
			session = HttpSession.get(sessionID);
		if (session != null)
			return session;
	...
	// **************************************************************************
	/**
	 * Returns the session ID specified by the client ("JSESSIONID" cookie). If
	 * the client did not specify a session ID, this method returns null. Use
	 * the isRequestedSessionIdValid() method to verify whether the session ID
	 * is valid.
	 */
	@Override
	 ...
	 * <p/>
	 * Attributes contain custom information about a request. Attributes are set
	 * programatically using the setAttribute() method and are typically used in
	 * conjunction with a RequestDispatcher. Attribute names should follow the
	 * same conventions as package names. The servlet specification reserves
	...
	// **************************************************************************
	/**
	 * Removes an attribute associated with this request. See getAttribute() and
	 * setAttribute() for more information.
	 */
	@Override
	 ...
	 * response. This server does not currently support RequestDispatcher so
	 ...
	 * 
	 * <pre>
	 * http://localhost:8080/MyServlet/Extra/Path/?abc=123
	 * </pre>
	 * 
	 ...
	 * any extra path information, or if the servlet container cannot translate
	 * the virtual path to a real path for any reason, this method returns a
	 * null.
	 */
	@Override
	 ...
	 * 
	 * <pre>
	 * http://localhost:8080/WebApplication/MyServlet/?abc=123
	 * </pre>
	 * <p/>
	 *
	 ...
	 * the servlet path in the servlet and implement logic to process the URLs
	 * accordingly.
	 */
	@Override
	 ...
	 * request header. If an Authenticator is not used to secure the servlet, a
	 * null is returned.
	 */
	@Override
	public String getAuthType() {
		if (authenticator != null)
			return authenticator.getAuthType();
		else
			return null;
	}
	...
	// **************************************************************************
	/**
	 * Returns an array representing the client credentials associated with this
	 * request. The first element in the array represents the username and the
	 * second element represents the password.
	 * <p/>
	 ...
	 * automatically after each http request to free up server resources. Do not
	...
	private byte[] getApplicationData() throws IOException {
		...
		// Read the next 5 bytes from the socket channel. This should contain
		// TLS
		// record information.
		...
		if (firstByte != 23)
			throw new IOException(
			        "Invalid record found when processing TLS application data. First byte is: " + firstByte);
	...
	private void printTLS(ByteBuffer buf) throws IOException {
		...
		try {
			while (true) {
				...
				String sslVersion = buf.get() + "." + buf.get(); // 0x0301 =>
				                                                 // 3.1 -
				                                                 // SSLv3/TLSv1
				...
				if (contentType.equals("change_cipher_spec"))
	 ...
	 *
	 * @param decrypt
	 *            Flag used to indicate whether to decrypt the data read from
	 *            the socket. This is required when processing SSL/TLS
	 *            application data. If true, the method will fetch the next
	 *            available TLS application record. Note that the TLS record
	 *            will rarely match the requested buffer size/capacity. In this
	 *            case, we will fetch n-number of TLS records from the client
	 *            until we have filled the buffer.
	 *
	 ...
	 * by the client and the selected cipher is valid, the client will send a
	 * reply that may include a certificate chain, a certificate verify message,
	 * change cipher spec request, and a client finished message. At this point,
	 * if everything goes well, the server will reply with a change cipher spec
	 * response and a server finished message. This will signal the end of a
	...
	private void initHandshake() throws ServletException, IOException {
		 ...
		 * encrypted data and handshake information. This buffer is filled with
		 * data read from the socket and emptied by SSLEngine.unwrap()
		 */
		...
		inNetData.rewind();
		recordHeader = null; // <--Null this out so we can process application
		                     // data!

		int x = 0;

		SSLEngineResult serverResult = null; // results from server's last
		                                     // operation
	 ...
	 */
	private HandshakeStatus runDelegatedTasks(SSLEngineResult result, SSLEngine engine) throws IOException {
