...
public class Directory implements Comparable {
	 ...
	 *            return files found in the current directory. <br/>
	 *
	 ...
	 *            directory, including any subdirectories. If false, the method
	 *            will simply return items found in the current directory. <br/>
	 *
	 *            Note that if the thread is interrupted for whatever reason
	 *            during a recursive search, the search will stop immediately.
	 *            Consequently, the returned array may be incomplete. You can
	 ...
	 * 
	 * 
	 * java.util.List<String> files = new java.util.ArrayList<String>(); try{
	 * String path = this.getPath();
	 * 
	 * 
	 * //Execute a ls command to get a directory listing String[] params = new
	 * String[]{"ls", "-ap", path}; javaxt.io.Shell cmd = new
	 * javaxt.io.Shell(params); java.util.List<String> output = cmd.getOutput();
	 * cmd.run(true);
	 * 
	 * 
	 * 
	 ...
	 * for (int i=0; i<arr.length; i++){ arr[i] = files.get(i); }
	 * 
	 * return arr; }
	 */
	 ...
	 *         directories found in this directory. Note that directory names
	 *         include a path separator which is used to distinguish files from
	 *         directories.
	 */
	private String[] dir() {
...
class DirectorySearch implements Runnable {
	...
	public void run() {
		...
		while (true) {
			 ...
			 * 
			 * if (activatedThreads.size()<numThreads){ updatePool(null); }
			 * else{ //System.out.println(this.getDirectoryID() + " activated "
			 * + activatedThreads.size() + " threads"); updateStatus(); } } }
			 * 
			 * return; }
			 */
...
// ******************************************************************************
/**
 * Used to watch changes made to files and folders (subdirectories) found in a
 * directory. Provides an option to monitor NTFS events on Windows-based
 * machines. This is an extremely efficient way to monitor a file system. Unlike
 * more traditional methods, this approach minimizes the amount of disk i/o
 * required and significantly reduces memory consumption. Note that this
 * approach only works on Windows 2000, XP, or later and you need a dynamic link
 * library called javaxt-core.dll. Again, this is just an option. For
 * non-Windows operation systems, the FileWatcher class will periodically scan
 * the file system for updates.
 *
 ******************************************************************************/
...
class FileSystemWatcher extends MLog implements Runnable {
	...
	public FileSystemWatcher(Directory directory) throws java.io.IOException {
		if (!directory.exists())
			throw new java.io.IOException("Directory not found.");
	...
	public final void run() {
		...
		if (!File.loadDLL()) {
			...
			try {
				long osWaitHandle = FileSystemWatcherNative.FindFirstChangeNotification(directory.getPath(),
				...
				// Process events. Note that the ReadDirectoryChangesW method
				// will
				...
				// I have no idea what this block of code does. Probably safe to
				// delete.
	...
	private void addEvent(String str) {
		...
		// Determine whether to update the events list when a file or folder is
		// modified
		if (action.equalsIgnoreCase("modify")) {
